...
def _read_colmap_pose(extrinsic_path, intrinsic_path, n_views=-1,llffhold=8): 
    ...
    if extrinsic_path.endswith('.txt') and intrinsic_path.endswith('.txt'):
...
def reconstruct(tmpdirname, model, device, image_size, input_files, silent=False, colmap_c2w_poses=None, intrinsics=None):
    ...
    if colmap_c2w_poses is not None:
        ...
        for i in range(len(colmap_c2w_poses)):
            ...
            if  np.abs(intrinsics[i][0,0] - intrinsics[i][1,1])>2:
                print(f"Warning: intrinsics for camera {i} are not square: {intrinsics[i][0,0]} != {intrinsics[i][1,1]}")
...
def get_pcd_from_sparse_view():
    ...
    for scene in [
        ...
        input_folder = f"/home/yxumich/datadisk/mipnerf-360/360_v2/{scene}/images_4/"
        ...
        extrinsic_path = f"/home/yxumich/datadisk/mipnerf-360/360_v2/{scene}/sparse/0/images.bin"
        intrinsic_path = f"/home/yxumich/datadisk/mipnerf-360/360_v2/{scene}/sparse/0/cameras.bin" 
