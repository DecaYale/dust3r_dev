...
def _read_colmap_pose(extrinsic_path, intrinsic_path, n_views=-1,llffhold=8): 
    from dust3r.utils.colmap_loader import read_extrinsics_binary, read_intrinsics_binary, qvec2rotmat, focal2fov
...
def _convert_scene_output_to_glb(outdir, imgs, pts3d, mask, focals, cams2world, cam_size=0.05,
                                 ...
                                 transparent_cams=False, silent=False, colmap_poses=None):  
    ...
    if colmap_poses is not None:
        ...
        for i in range(len(colmap_poses)):
            colmap_cam2world = np.linalg.inv(colmap_poses[i])
    ...
    # add each camera
    for i, pose_c2w in enumerate(aligned_colmap_c2w_list):
        if isinstance(cam_color, list):
            camera_edge_color = cam_color[i]
        else:
            camera_edge_color = cam_color or CAM_COLORS[i % len(CAM_COLORS)]
...
def get_3D_model_from_scene(outdir, silent, scene, min_conf_thr=3, as_pointcloud=False, mask_sky=False,
    ...
    msk = to_numpy(scene.get_masks())
    return _convert_scene_output_to_glb(outdir, rgbimg, pts3d, msk, focals, cams2world, as_pointcloud=as_pointcloud,
                                        transparent_cams=transparent_cams, cam_size=cam_size, silent=silent, colmap_poses=colmap_poses)
...
def get_reconstructed_scene(outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
    ...
    # extrinsic_path = f"{os.path.dirname(filelist[0])}/../colmap/sparse/images.bin"
    # intrinsic_path = f"{os.path.dirname(filelist[0])}/../colmap/sparse/cameras.bin" 
    ...
    train_cam_infos, test_cam_infos= _read_colmap_pose(extrinsic_path=extrinsic_path, intrinsic_path=intrinsic_path, n_views=len(imgs),llffhold=8)
    colmap_poses = []
    for cam_info in train_cam_infos:  
        cam2world = np.eye(4)
        cam2world[:3,:3] = cam_info["R"]
        cam2world[:3,3] = cam_info["T"]
        colmap_poses.append(cam2world)
    colmap_poses = np.stack(colmap_poses)
    import pdb; pdb.set_trace()
    ...
    outfile = get_3D_model_from_scene(outdir, silent, scene, min_conf_thr, as_pointcloud, mask_sky,
                                      clean_depth, transparent_cams, cam_size, colmap_poses=colmap_poses)
...
def main_demo(tmpdirname, model, device, image_size, server_name, server_port, silent=False):
    ...
    with gradio.Blocks(css=""".gradio-container {margin: 0 !important; min-width: 100%};""", title="DUSt3R Demo") as demo:
        ...
        with gradio.Column():
            ...
            # colmap_extrinsic_file = gradio.File(file_count="single", label='colmap_extrinsic')
            # colmap_intrinsic_file = gradio.File(file_count="single", label='colmap_intrinsic')
            ...
            #read colmap poses
            # import pdb; pdb.set_trace()
            # train_cam_infos, test_cam_infos = _read_colmap_pose(extrinsic_path=colmap_extrinsic_file, intrinsic_path=colmap_intrinsic_file, n_views=len(inputfiles),llffhold=8)
            # colmap_poses = []
            # for cam_info in train_cam_infos:  
            #     cam2world = np.eye(4)
            #     cam2world[:3,:3] = cam_info["R"]
